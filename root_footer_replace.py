...
def main():
    # Important root-level files to prioritize
    priority_files = [
        "contact.html",
        "services.html", 
        "pay-dues.html",
    ...
    # Find all root level HTML files, prioritizing the important ones
    all_files = glob.glob("*.html")
    
    # Filter out files that already have the new footer and non-important files
    files_to_update = []
    for filepath in all_files:
        # Skip certain files that are likely templates or not user-facing
        if any(skip in filepath.lower() for skip in ['test', 'template', 'snippet', 'menu', 'google', 'offline', 'perfect-footer']):
            continue
    ...
    priority_to_update = [f for f in files_to_update if f in priority_files]
    other_to_update = [f for f in files_to_update if f not in priority_files]
    
    final_list = priority_to_update + other_to_update
    
    print(f"Found {len(final_list)} root-level HTML files to update")
    print(f"Priority files: {priority_to_update}")
    print(f"Other files: {other_to_update}")

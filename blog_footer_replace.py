...
def replace_footer_in_blog_file(filepath):
    """Replace the old footer with the new footer in a single blog file."""
    try:
        ...
        # Find the old footer pattern and replace everything from <footer> to </footer>
        # Use regex to match the footer section including all the complex content
        footer_pattern = r'<footer>.*?</footer>'
        
        # Use multiline and dotall flags to match across lines
        if re.search(footer_pattern, content, re.DOTALL | re.MULTILINE):
            new_content = re.sub(footer_pattern, new_footer, content, flags=re.DOTALL | re.MULTILINE)
...
def main():
    ...
    for filepath in files:
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                content = f.read()
            if 'background: #2C3E50' not in content:
                files_to_update.append(filepath)
        except Exception as e:
            print(f"Error checking {filepath}: {e}")
    
    print(f"Found {len(files_to_update)} blog files to update")
    
    updated_count = 0
    for filepath in files_to_update:
        if replace_footer_in_blog_file(filepath):
            updated_count += 1

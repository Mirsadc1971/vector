...
# Perfect footer CSS content
FOOTER_CSS='        /* PERFECT FOOTER CSS - EXACT SPECIFICATIONS */
        footer {
            background: #2c3e50;
            color: white;
            line-height: 1.6;
...
# Perfect footer HTML content
FOOTER_HTML='    <!-- PERFECT FOOTER HTML - EXACT SPECIFICATIONS -->
    <footer>
...
# Find all files with simple-footer and update them
find "property-management" -name "*.html" -exec grep -l "simple-footer" {} \; | grep -v "index-old.html" | grep -v "morton-grove/index.html.backup" | while read file; do
    echo "Processing $file..."
    
    # Create backup
    cp "$file" "$file.backup"
    
    # Replace simple footer CSS pattern
    sed -i '/\.simple-footer {/,/\.simple-footer a:hover {/{
        /\.simple-footer {/c\
'"$FOOTER_CSS"'
        t
        d
...
'"$FOOTER_CSS"'
    ...
    # Replace simple footer HTML pattern
    sed -i '/<footer class="simple-footer">/,/<\/footer>/{
        /<footer class="simple-footer">/c\
'"$FOOTER_HTML"'
        t
        d
    }' "$file"
...
echo "All footer updates completed!"